...
function vacio() {
  return { emails: {}, checks: {}, notas: {}, ultimoMes: null, ultimoRespaldo: null, asociaciones: {}, segmentosManual: {} };
}
...
function cargarEstado() {
  try {
    ...
    return {
      ...
      asociaciones: s.asociaciones || {},
      segmentosManual: s.segmentosManual || {}
    };
  } catch (e) {
...
/* Devuelve los indicadores derivados de las métricas del envío: si el mail
   está asociado a una campaña real, enviados/aperturas/clics/bajas vienen
   de ahí (en vivo); conversión y $ generados siguen siendo a mano porque
   ninguna de las dos APIs los tiene. */
function calcular(email) {
  const m = datosEmail(email.id).m;
  const asociada = campanaAsociada(email);
  const env = asociada ? (asociada.destinatarios ?? null) : num(m.enviados);
  const ap = asociada ? (asociada.aperturasUnicas ?? null) : num(m.aperturas);
  const cl = asociada ? (asociada.clicsUnicos ?? null) : num(m.clics);
  const ba = asociada ? (asociada.bajas ?? null) : num(m.bajas);
  const cv = num(m.conv), mo = num(m.monto);
...
const CAMPOS_AUTO = { fecha: 1, enviados: 1, aperturas: 1, clics: 1, bajas: 1, rebotes: 1 };

function valorAutoDe(asociada, clave) {
  switch (clave) {
    case 'fecha': return asociada.fechaEnvio ? asociada.fechaEnvio.slice(0, 10) : null;
    case 'enviados': return asociada.destinatarios ?? null;
    case 'aperturas': return asociada.aperturasUnicas ?? null;
    case 'clics': return asociada.clicsUnicos ?? null;
    case 'bajas': return asociada.bajas ?? null;
    case 'rebotes': return rebotesDe(asociada);
    default: return null;
  }
}
...
function tarjetaEmail(email) {
  ...
  const campos = CAMPOS.map(c => {
    ...
    if (asociada && CAMPOS_AUTO[c.k]) {
      const v = valorAutoDe(asociada, c.k);
      const mostrado = c.k === 'fecha' ? (fmtFecha(v) || '—') : fmtNum(v);
      return '<div class="campo-auto"><span>' + esc(etiqueta) + '</span><b>' + esc(mostrado) + '</b></div>';
    }
  ...
  const candidatas = asociada ? [] : candidatasPara(email);
  const asociacionHTML = asociada
    ? '<span class="fc-seg fc-seg-' + seg.color + '">🔗 ' +
        esc(fmtFecha(asociada.fechaEnvio ? asociada.fechaEnvio.slice(0, 10) : null) || 'sin fecha') +
        ' · ' + esc(asociada.plataforma) + '</span>' +
      '<button class="btn btn-chico" data-accion="desasociar">Desasociar</button>'
    : candidatas.length
  ...
  '<article class="tarjeta seg-' + seg.color + (fueEditado(email) ? ' editado' : '') + '" data-email="' + email.id + '">' +
    ...
    '<div class="tarjeta-body">' +
      ...
      '<div class="bloque">' +
        '<div class="bloque-header"><span class="bloque-titulo">Lo que dio el envío</span>' + asociacionHTML + '</div>' +
        '<div class="bloque-body">' +
...
function render() {
  ...
  /* También hace falta en meses del plan: son las candidatas para asociar
     un mail a la campaña real que lo mandó. Sin bloquear el render — las
     tarjetas se ven igual mientras carga, la asociación aparece sola
     cuando `historico` esté listo (cargarHistorico() vuelve a renderizar). */
  if (!historico && !historicoCargando) cargarHistorico(false);
...
function conectarEventos() {
  ...
  document.querySelectorAll('.tarjeta').forEach(tarjeta => {
    ...
    const selAsociar = tarjeta.querySelector('[data-asociar]');
    if (selAsociar) {
      selAsociar.addEventListener('change', () => {
        if (!selAsociar.value) return;
        asociar(email, selAsociar.value);
        toast('Campaña asociada — las métricas se traen solas de acá en más');
      });
    }
  });
...
/* render() actualiza el mes actual (si es "pasado") y renderMetricasHTML()
   la pestaña Métricas — se llaman las dos sin condicionar por cuál está
   visible, es más simple que rastrear la vista activa y no cuesta nada
   redibujar un contenedor oculto. */
function redibujarVistasConHistorico() {
  render();
  renderMetricasHTML();
}

async function cargarHistorico(forzar) {
  if (historico && !forzar) { redibujarVistasConHistorico(); return; }
  historicoCargando = true;
  redibujarVistasConHistorico();
  try {
...
function rebotesDe(c) {
  return c.rebotes != null ? c.rebotes : (c.rebotesDuros != null || c.rebotesBlandos != null
    ? (c.rebotesDuros || 0) + (c.rebotesBlandos || 0) : null);
}

function filaCampana(c) {
  const rebotes = rebotesDe(c);
  ...
  const etiquetaSeg = seg
    ...
    : '<select class="select-chico" data-marcar-segmento="' + esc(claveCampana(c)) + '">' +
        '<option value="">' + (sinIdentificar ? '⚠️ ' + esc(sinIdentificar) : 'Marcar segmento…') + '</option>' +
        Object.keys(SEGMENTOS).map(k =>
          '<option value="' + k + '">' + SEGMENTOS[k].icono + ' ' + esc(SEGMENTOS[k].nombre) + '</option>').join('') +
      '</select>';
...
function renderMesPasado(mes) {
  ...
  const deEsteMes = c => c.fechaEnvio && c.fechaEnvio.slice(0, 7) === clave;
  const todas = todasLasCampanas().filter(deEsteMes);
  const mc = todas.filter(c => c.plataforma === 'Mailchimp');
  const eo = todas.filter(c => c.plataforma === 'EmailOctopus');
  ...
  document.querySelectorAll('#contenido [data-marcar-segmento]').forEach(sel => {
    sel.addEventListener('change', () => {
      const clave = sel.dataset.marcarSegmento;
      if (sel.value) estado.segmentosManual[clave] = sel.value; else delete estado.segmentosManual[clave];
      guardar();
      render();
      toast(sel.value ? 'Segmento marcado' : 'Segmento sin marcar');
    });
  });
...
/* Todo lo mandado desde enero 2026 (Mailchimp + EmailOctopus), general y
   por segmento. Mismo fetch/caché de 5h que los meses pasados — comparten
   `historico`, así que si ya se cargó en otra vista no se vuelve a pedir. */

/* EmailOctopus no expone tag/segmento por campaña en su API (a diferencia
   de Mailchimp) — confirmado con datos reales el 2026-08-07. Para esos
   casos (y para cualquier tag de Mailchimp que no esté en el mapeo) el
   segmento se puede marcar a mano en el panel; esa marca pisa lo que haya
   traído la API y de ahí se propaga sola a Métricas y a la asociación. */
function todasLasCampanas() {
  if (!historico || historico.error) return [];
  const mc = (historico.mailchimp || []).map(c => Object.assign({ plataforma: 'Mailchimp' }, c));
  const eo = (historico.emailoctopus || []).map(c => Object.assign({ plataforma: 'EmailOctopus' }, c));
  return mc.concat(eo).map(c => {
    const clave = c.plataforma + ':' + c.id;
    const manual = estado.segmentosManual[clave];
    return manual ? Object.assign({}, c, { segmento: manual }) : c;
  });
}

/* --------------------------------------------------------- ASOCIACIÓN --- */

/* Vincular un mail del plan con la campaña real que lo mandó: a partir de
   ahí, aperturas/clics/bajas/rebotes/enviados se traen solos de la API en
   vez de cargarse a mano. La elige el usuario (no se adivina por fecha):
   puede haber más de un envío al mismo segmento en el mismo mes. */

const claveCampana = c => c.plataforma + ':' + c.id;

function campanaAsociada(email) {
  const clave = estado.asociaciones[email.id];
  if (!clave) return null;
  return todasLasCampanas().find(c => claveCampana(c) === clave) || null;
}

/* Campañas del mismo segmento que todavía no están asociadas a NINGÚN mail
   (ni a este ni a otro) — evita ofrecer dos veces la misma campaña. */
function candidatasPara(email) {
  const usadas = new Set(Object.values(estado.asociaciones));
  return todasLasCampanas()
    .filter(c => c.segmento === email.seg && !usadas.has(claveCampana(c)))
    .sort((a, b) => (b.fechaEnvio || '').localeCompare(a.fechaEnvio || ''));
}

function asociar(email, clave) {
  if (clave) estado.asociaciones[email.id] = clave; else delete estado.asociaciones[email.id];
  guardar();
  render();
}
...
function filaSegmentoMetricas(g) {
  const seg = g.k === '_sin' ? null : SEGMENTOS[g.k];
  const nombre = seg ? seg.icono + ' ' + seg.nombre : 'Sin identificar';
  const clase = seg ? 'fc-seg-' + seg.color : 'fc-seg-desconocido';
  const ap = promedio(g.item.map(c => c.tasaApertura));
  const cl = promedio(g.item.map(c => c.tasaClics));
  return '<div class="seg-fila">' +
    '<span class="fc-seg ' + clase + '">' + esc(nombre) + '</span>' +
    '<span class="seg-fila-n">' + g.item.length + (g.item.length === 1 ? ' campaña' : ' campañas') + '</span>' +
    '<span class="seg-fila-m"><b>' + fmtPct(ap !== null ? ap * 100 : null) + '</b> apertura prom.</span>' +
    '<span class="seg-fila-m"><b>' + fmtPct(cl !== null ? cl * 100 : null) + '</b> clics prom.</span>' +
  '</div>';
}

function filaRankingMetricas(c) {
  const seg = c.segmento ? SEGMENTOS[c.segmento] : null;
  const sinIdentificar = c.tagCrudo || c.listaNombre;
  const etiquetaSeg = seg
    ? '<span class="fc-seg fc-seg-' + seg.color + '">' + seg.icono + ' ' + esc(seg.nombre) + '</span>'
  ...
  return '<div class="fila-campana">' +
    ...
    '</div>' +
    '<div class="fc-metricas">' +
      '<div class="fc-m"><b>' + fmtPct(c.tasaApertura * 100) + '</b><span>Apertura</span></div>' +
      '<div class="fc-m"><b>' + fmtNum(c.aperturasUnicas) + '</b><span>Aperturas</span></div>' +
      '<div class="fc-m"><b>' + fmtNum(c.clicsUnicos) + '</b><span>Clics</span></div>' +
    '</div>' +
  '</div>';
}

function renderMetricasHTML() {
  const cont = document.getElementById('contenido-metricas');
  if (!cont) return;

  if (historicoCargando) {
    cont.innerHTML = '<div class="foco-mes"><div class="foco-texto"><h2>Métricas de mails</h2>' +
      '<p>Cargando campañas reales de Mailchimp y EmailOctopus…</p></div></div>';
    return;
  }
  ...
  if (!historico || historico.error) {
    ...
    cont.innerHTML = '<div class="foco-mes"><div class="foco-texto"><h2>Métricas de mails</h2>' +
      '<p>No se pudo conectar con <code>/api/metricas</code>' + detalle + '. Esto solo funciona abriendo el ' +
      'dashboard publicado en Vercel — no en GitHub Pages ni con el archivo local.</p>' +
      '<button class="btn btn-chico" id="btn-reintentar-metricas" style="margin-top:10px">Reintentar</button>' +
      '</div></div>';
    const btn = document.getElementById('btn-reintentar-metricas');
    if (btn) btn.addEventListener('click', () => cargarHistorico(true));
    return;
  }

  const todas = todasLasCampanas();
  const errores = (historico.errores || []).length
    ? '<div class="nota-item alerta">' + historico.errores.map(esc).join('<br>') + '</div>' : '';

  if (!todas.length) {
    cont.innerHTML = '<div class="foco-mes"><div class="foco-texto"><h2>Métricas de mails</h2>' +
      '<p>Todavía no hay campañas enviadas desde enero de 2026.</p></div></div>' + errores;
    return;
  }

  const totDest = todas.reduce((a, c) => a + (c.destinatarios || 0), 0);
  const apProm = promedio(todas.map(c => c.tasaApertura));
  const clProm = promedio(todas.map(c => c.tasaClics));
  const generado = historico.generadoEn ? new Date(historico.generadoEn).toLocaleString('es-UY') : '—';

  const grupos = Object.keys(SEGMENTOS).concat(['_sin'])
    .map(k => ({ k, item: todas.filter(c => (c.segmento || '_sin') === k) }))
    .filter(g => g.item.length);

  const conTasa = todas.filter(c => c.tasaApertura !== null && c.tasaApertura !== undefined);
  const mejores = conTasa.slice().sort((a, b) => b.tasaApertura - a.tasaApertura).slice(0, 5);
  const peores = conTasa.length > 5
    ? conTasa.slice().sort((a, b) => a.tasaApertura - b.tasaApertura).slice(0, 5)
    : [];

  cont.innerHTML =
    '<div class="foco-mes">' +
      '<div class="foco-texto">' +
        '<h2>Métricas de mails</h2>' +
        '<p>Todo lo mandado desde enero de 2026 por Mailchimp y EmailOctopus, en vivo. Se actualiza solo, ' +
        'como máximo cada 5 horas.</p>' +
      '</div>' +
      '<div class="resumen-mes">' +
        '<div class="resumen-item"><div class="resumen-num">' + todas.length + '</div><div class="resumen-lbl">Campañas</div></div>' +
        '<div class="resumen-item"><div class="resumen-num">' + fmtNum(totDest) + '</div><div class="resumen-lbl">Destinatarios</div></div>' +
        '<div class="resumen-item"><div class="resumen-num">' + fmtPct(apProm !== null ? apProm * 100 : null) + '</div><div class="resumen-lbl">Apertura prom.</div></div>' +
        '<div class="resumen-item"><div class="resumen-num">' + fmtPct(clProm !== null ? clProm * 100 : null) + '</div><div class="resumen-lbl">CTR prom.</div></div>' +
      '</div>' +
    '</div>' +
    errores +
    '<section class="seccion">' +
      '<div class="seccion-header"><h3>Por segmento</h3></div>' +
      '<div class="lista-segmentos">' + grupos.map(filaSegmentoMetricas).join('') + '</div>' +
    '</section>' +
    (mejores.length
      ? '<section class="seccion"><div class="seccion-header"><h3>🏆 Lo que mejor funcionó</h3></div>' +
        '<div class="lista-campanas">' + mejores.map(filaRankingMetricas).join('') + '</div></section>'
      : '') +
    (peores.length
      ? '<section class="seccion"><div class="seccion-header"><h3>⚠️ Lo que menos funcionó</h3></div>' +
        '<div class="lista-campanas">' + peores.map(filaRankingMetricas).join('') + '</div></section>'
      : '') +
    '<p class="fc-generado">Última consulta: ' + esc(generado) +
      ' · <button class="btn btn-chico" id="btn-reintentar-metricas">Actualizar</button></p>';

  const btn = document.getElementById('btn-reintentar-metricas');
  if (btn) btn.addEventListener('click', () => cargarHistorico(true));

  anim(document.querySelectorAll('#contenido-metricas .foco-mes, #contenido-metricas .seccion'),
    { opacity: [0, 1], transform: ['translateY(14px)', 'translateY(0)'] },
    { duration: 0.45, delay: escalonar(0.05), ease: SALIDA });
}
...
function verVista(cual) {
  const vistas = {
    ...
    metricas: document.getElementById('contenido-metricas')
  };
